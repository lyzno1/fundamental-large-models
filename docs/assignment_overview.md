...
- 基于 `uv` 初始化的 Python 项目结构，满足课程要求的目录布局。仓库附带 `scripts/prepare_tiny_shakespeare.py`，首次运行会联网从 Karpathy `char-rnn` 仓库拉取 Tiny Shakespeare 原始文本并缓存至 `data/tiny_shakespeare/`（脚本会自动写出 zip 压缩包，可在提交前纳入仓库）。
